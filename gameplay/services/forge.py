...
def has_active_forging(manor: Manor) -> bool:
    ...
    Returns:
        是否有锻造中的装备
    """
    return manor.equipment_productions.filter(
...
def get_equipment_by_category(manor: Manor) -> Dict[str, List[Dict[str, Any]]]:
    """
    按类别分组获取装备选项。
...
def _schedule_forging_completion(production: EquipmentProduction, eta_seconds: int) -> None:
    ...
    db_transaction.on_commit(
        lambda: complete_equipment_forging.apply_async(args=[production.id], countdown=countdown)
    )
...
def get_active_forgings(manor: Manor) -> List[EquipmentProduction]:
    ...
    """
    return list(
        manor.equipment_productions.filter(
            status=EquipmentProduction.Status.FORGING
