...
def _cleanup_local_fallback_cache(now_monotonic: float, stale_threshold: float) -> None:
    ...
    # 第一轮：删除过期条目
    stale_keys = [key for key, ts in _LOCAL_REFRESH_FALLBACK.items() if ts < stale_before]
    ...
    if len(_LOCAL_REFRESH_FALLBACK) > _LOCAL_REFRESH_FALLBACK_MAX_SIZE:
        # 按时间戳排序，淘汰最旧的条目
        sorted_items = sorted(_LOCAL_REFRESH_FALLBACK.items(), key=lambda item: item[1])
...
def bootstrap_buildings(manor: Manor) -> None:
    ...
    buildings_to_create = [Building(manor=manor, building_type=bt) for bt in building_types]
...
def ensure_buildings_exist(manor: Manor) -> None:
    ...
    if missing:
        buildings_to_create = [Building(manor=manor, building_type=bt) for bt in missing]
