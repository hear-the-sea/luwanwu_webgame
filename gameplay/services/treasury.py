...
def get_treasury_capacity(manor: Manor) -> int:
    ...
    """
    treasury = manor.buildings.select_related("building_type").filter(
        building_type__key=BuildingKeys.TREASURY
    ).first()
...
def move_item_to_treasury(manor: Manor, item_id: int, quantity: int) -> None:
    ...
    # 获取仓库中的物品
    warehouse_item = InventoryItem.objects.select_for_update().filter(
        id=item_id,
        manor=manor,
        storage_location=InventoryItem.StorageLocation.WAREHOUSE
    ...
    if warehouse_item.template.key == "gold_bar":
        ...
        if available < quantity:
            raise InsufficientStockError(
                warehouse_item.template.name, quantity, available
            )
    elif warehouse_item.quantity < quantity:
        raise InsufficientStockError(
            warehouse_item.template.name, quantity, warehouse_item.quantity
        )
    ...
    if treasury_used + item_space > treasury_capacity:
        raise InsufficientSpaceError(
            "treasury", treasury_capacity - treasury_used, item_space
        )
...
def move_item_to_warehouse(manor: Manor, item_id: int, quantity: int) -> None:
    ...
    """
    # 获取藏宝阁中的物品
    treasury_item = InventoryItem.objects.select_for_update().filter(
        id=item_id,
        manor=manor,
        storage_location=InventoryItem.StorageLocation.TREASURY
    ).select_related("template").first()
    ...
    if treasury_item.quantity < quantity:
        raise InsufficientStockError(
            treasury_item.template.name, quantity, treasury_item.quantity
        )
