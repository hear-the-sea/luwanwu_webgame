...
def refresh_scout_records(manor: Manor) -> None:
    """刷新庄园的侦察记录状态"""
    now = timezone.now()

    # 处理去程到达的侦察（SCOUTING -> RETURNING）
    scouting_records = ScoutRecord.objects.filter(
        attacker=manor,
        status=ScoutRecord.Status.SCOUTING,
    ...
    # 处理返程完成的侦察（RETURNING -> SUCCESS/FAILED）
    returning_records = ScoutRecord.objects.filter(
        attacker=manor,
