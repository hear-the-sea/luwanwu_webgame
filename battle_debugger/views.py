...
from .config import ConfigLoader, BattleConfig, PartyConfig, GuestConfig
...
def index(request):
    ...
    return render(request, "battle_debugger/index.html", {
        "presets": preset_info,
    })
...
def preset_detail(request, preset_name):
    ...
    try:
        ...
        return render(request, "battle_debugger/error.html", {
            "error": f"预设配置不存在: {preset_name}"
        })

    return render(request, "battle_debugger/preset_detail.html", {
...
def simulate(request):
    ...
    if request.method == "GET":
        ...
        presets = loader.list_presets()
        preset_info = [
            {'name': p, 'title': loader.get_preset_info(p).get('name', p)}
            for p in presets
        ]
        ...
        tunable_params = [
            ...
            {'key': 'troop_attack_divisor_vs_guest', 'label': '小兵打门客攻击除数', 'default': 4.0, 'min': 1, 'max': 10, 'step': 0.1},
            {'key': 'troop_attack_divisor_vs_troop', 'label': '小兵打小兵攻击除数', 'default': 1.0, 'min': 0.5, 'max': 5, 'step': 0.1},
            {'key': 'troop_defense_divisor', 'label': '小兵防御除数', 'default': 2.0, 'min': 0.5, 'max': 10, 'step': 0.1},
        ...
        return render(request, "battle_debugger/simulate.html", {
            "presets": preset_info,
    ...
    try:
        ...
        cache.set(f'battle_result_{result_id}', {
            'config': config,
            'results': results,
            'preset_name': preset_name,
        }, timeout=3600)  # 1小时过期
...
def result_detail(request, result_id):
    ...
    return render(request, "battle_debugger/result_detail.html", {
        "result_id": result_id,
        "config": config,
        "results": results,
        "stats": stats,
...
def tune(request):
    ...
    if request.method == "GET":
        ...
        return render(request, "battle_debugger/tune.html", {
            "presets": preset_info,
            "tunable_params": tunable_params,
        })
    ...
    try:
        ...
        return render(request, "battle_debugger/tune_result.html", {
            "preset_name": preset_name,
...
def custom_config(request):
    ...
    if request.method == "GET":
        ...
        return render(request, "battle_debugger/custom_config.html", {
            "guests": guests,
            "skills": skills,
            "troops": troops,
            "tunable_params": tunable_params,
