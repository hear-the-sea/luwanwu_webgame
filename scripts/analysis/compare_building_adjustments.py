...
for plan in [current, plan1, plan2, plan3]:
    result = calculate_building_total(
        plan["base_cost"], plan["cost_growth"], plan["base_time"], plan["time_growth"], plan["max_level"]
    )
    print(f"\n{plan['name']} (Lv{plan['max_level']})")
    print(
        f"  - 参数: base_time={plan['base_time']}, time_growth={plan['time_growth']}, cost_growth={plan['cost_growth']}"
    )
...
for plan in [current, plan1, plan2]:
    ...
    print(f"\n{plan['name']} (Lv{plan['max_level']})")
    print(
        f"  - 参数: base_cost={plan['base_cost']['silver']}, cost_growth={plan['cost_growth']}, base_time={plan['base_time']}, time_growth={plan['time_growth']}"
    )
    print(f"  - 总时长: {result['time_days']:.2f}天 ({result['time_hours']:.1f}小时)")
    print(f"  - 总成本: {result['silver']:,}银两")
...
for plan in [current, plan1, plan2, plan3]:
    ...
    print(
        f"         cost_growth={plan['cost_growth']}, base_time={plan['base_time']}, time_growth={plan['time_growth']}"
    )
