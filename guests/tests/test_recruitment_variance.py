...
class TestRecruitmentVariance(TestCase):
    ...
    def test_total_points_unchanged(self):
        ...
        for _ in range(50):  # 多次测试
            result = apply_recruitment_variance(self.green_military_template, "green", "military", rng)
            ...
            self.assertEqual(result_total, original_total, f"四维总点应保持{original_total}，但得到{result_total}")
    ...
    def test_single_attribute_bounds(self):
        ...
        for _ in range(50):
            ...
            for attr in ["force", "intellect", "defense", "agility"]:
                ...
                self.assertGreaterEqual(result_value, min_value, f"{attr}={result_value}低于最小值{min_value}")

                # 约束2：不超过112%
                max_value = int(base_value * ATTRIBUTE_VARIANCE_CONFIG["max_ratio"])
                self.assertLessEqual(result_value, max_value, f"{attr}={result_value}超过最大值{max_value}")

                # 约束3：可成长属性 < 100
                self.assertLess(
                    result_value, MAX_GROWABLE_ATTRIBUTE, f"{attr}={result_value}超过硬上限{MAX_GROWABLE_ATTRIBUTE}"
                )
    ...
    def test_luck_variance(self):
        ...
        for luck in luck_values:
            self.assertGreaterEqual(
                luck, base_luck - ATTRIBUTE_VARIANCE_CONFIG["luck_deviation"], f"运势{luck}低于下限"
            )
            self.assertLessEqual(luck, base_luck + ATTRIBUTE_VARIANCE_CONFIG["luck_deviation"], f"运势{luck}超过上限")

        # 检查运势确实有分布（不是全部相同）
        unique_luck = set(luck_values)
        self.assertGreater(len(unique_luck), 3, "运势波动范围过小，应该有多种不同值")
    ...
    def test_attribute_diversity(self):
        ...
        self.assertGreater(len(unique_combos), 10, f"20次生成应该有超过10种不同组合，实际{len(unique_combos)}种")
    ...
    def test_orange_civil_high_stats(self):
        ...
        for _ in range(50):
            result = apply_recruitment_variance(self.orange_civil_template, "orange", "civil", rng)

            # 橙色文官智力75、防御73，波动后可能接近上限
            for attr in ["force", "intellect", "defense", "agility"]:
                self.assertLess(result[attr], MAX_GROWABLE_ATTRIBUTE, f"橙色文官{attr}={result[attr]}超过硬上限")

    def test_deterministic_with_seed(self):
        """测试：相同种子产生相同结果"""
        rng1 = random.Random(12345)
        result1 = apply_recruitment_variance(self.green_military_template, "green", "military", rng1)

        rng2 = random.Random(12345)
        result2 = apply_recruitment_variance(self.green_military_template, "green", "military", rng2)
    ...
    def test_talent_grade_calculation(self):
        ...
        result = apply_recruitment_variance(self.green_military_template, "green", "military", random.Random(42))

        base_total = sum(self.green_military_template[attr] for attr in ["force", "intellect", "defense", "agility"])
    ...
    def test_minimum_attribute_values(self):
        ...
        for _ in range(50):
            ...
            for attr, value in result.items():
                self.assertGreaterEqual(value, 1, f"属性{attr}={value}不应小于1")
    ...
    def test_extreme_distribution_prevented(self):
        ...
        for _ in range(50):
            ...
            for attr in ["force", "intellect", "defense", "agility"]:
                ...
                self.assertGreaterEqual(ratio, 0.85, f"{attr}偏离模板过低：{value}/{base}={ratio:.2f}")
                self.assertLessEqual(ratio, 1.15, f"{attr}偏离模板过高：{value}/{base}={ratio:.2f}")
    ...
    def test_statistical_distribution(self):
        ...
        # 平均值应该接近模板值（允许±2的偏差）
        self.assertAlmostEqual(avg_force, base_force, delta=2.0, msg=f"武力平均值{avg_force}应接近模板值{base_force}")

        # 应该有足够的分散度
        variance = sum((x - avg_force) ** 2 for x in force_values) / len(force_values)
        std_dev = variance**0.5

        self.assertGreater(std_dev, 0.5, "武力标准差应该大于0.5，表明有足够的随机性")
