...
- `mission / raid / guest recruitment` 的真实并发语义测试仍然不够；虽然 `mission` 发起/撤退、refresh dispatch / worker 消费，`raid` 发起、`guest recruitment` 发起已覆盖首批关键竞争与派发语义，但更多 refresh 补偿链路和其它 worker 场景仍是缺口。
