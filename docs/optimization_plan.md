...
- `websocket/consumers/world_chat.py`、`config/asgi.py` 与 `battle/management/commands/load_troop_templates.py` 这批阶段 3 尾部漏口也已补齐：世界频道发布失败路径已改为“基础设施故障显式降级、补偿后返回；编程错误补偿后继续冒泡”的 `finally + sys.exc_info()` 收口模式，`ASGI` 路由导入与测试环境 Celery app 更新则只对白名单内的 `ImportError / AppRegistryNotReady / OSError / RuntimeError` 降级，本地导入兵种头像也已改为只对显式 `OSError` 做 warning；`except Exception` 的生产代码扫描当前重新回到 `0`，并补了 `tests/test_world_chat_consumer.py`、`tests/test_asgi.py`、`tests/test_management_command_validation.py` 回归约束这批边界不回退。
- `common/utils/random_utils.py` 的共享加权抽样入口也已退出 legacy `ValueError` 业务语义：`weighted_random_choice()` 在 `items / weights` 长度不一致或总权重非正时，当前改走显式 `AssertionError` 内部契约错误，不再继续把裸 `ValueError` 作为跨层失败语义带进招募等业务链路；同时已补 `tests/test_random_utils.py` 约束这两类坏输入继续直接暴露契约错误。
...
- `guilds/services/member.py` 的成员流转主链也已继续按阶段 3 收口：入帮申请、审批、拒绝、退帮、踢人、任命/罢免管理员与转让帮主这批入口当前不再把业务失败统一落成裸 `ValueError`，而是改走显式 `GuildMembershipError / GuildPermissionError`；这样 `guilds` 成员域开始具备和 `contribution / technology / warehouse` 一致的异常层次，避免服务层继续沿用模糊的 legacy `ValueError` 业务语义。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_guilds.py tests/test_guild_view_helpers.py -q` 为 `39 passed`，用于约束公会成员流转主链从 `ValueError` 切到显式 `GuildMembershipError / GuildPermissionError` 后，不会破坏现有页面 helper、通知 follow-up 和成员状态流转语义。
- `guilds/services/guild.py` 的创建/升级/改简介/解散主链也已继续按阶段 3 收口：帮会名称校验、创建时的金条/重名检查、升级时的等级/金条校验，以及简介更新与解散权限校验当前不再把业务失败统一落成裸 `ValueError`，而是改走显式 `GuildValidationError / GuildMembershipError / GuildPermissionError`；这样 `guilds` 的帮会主链开始具备和成员域一致的异常层次，避免服务层继续沿用模糊的 legacy `ValueError` 业务语义。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_guilds.py -k "create_guild or upgrade_guild or update_guild_info or disband_guild" -q` 为 `10 passed, 16 deselected`，用于约束公会创建/升级/改简介/解散主链从 `ValueError` 切到显式 `GuildValidationError / GuildPermissionError` 后，不会破坏原有业务校验和事务外 follow-up 语义。
- `guilds/services/hero_pool.py` 的门客池/出战名单主链也已继续按阶段 3 收口：槽位/门客参数校验、门客池提交与移除、出战名单增删这批入口当前不再把业务失败统一落成裸 `ValueError`，而是改走显式 `GuildValidationError / GuildMembershipError / GuildPermissionError`；这样 `guilds` 的 hero-pool 主链也开始具备与成员域、帮会主链一致的异常层次，避免服务层继续沿用模糊的 legacy `ValueError` 业务语义。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_guild_hero_pool.py tests/test_guild_hero_pool_views.py -q` 为 `13 passed`，用于约束公会门客池/出战名单主链从 `ValueError` 切到显式 `GuildValidationError / GuildMembershipError / GuildPermissionError` 后，不会破坏原有门客池冷却、阵容上限和页面提示语义。
- `guilds/services/technology.py` 与 `guilds/views/helpers.py` 的剩余 legacy 兼容层也已继续按阶段 3 收口：科技升级入口当前不再依赖 `get_active_membership()` 的旧 `ValueError` 语义，而是显式接住 `GuildMembershipError` 后统一翻译成 `GuildTechnologyError`；与此同时，`execute_guild_action()` 已移除 `ValueError` 兜底，只继续把 `GameError` 视为业务失败，避免 `guilds` 页面 helper 继续把 legacy / 编程错误伪装成普通用户提示。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_guild_view_helpers.py tests/test_guilds_technology_service.py -q` 为 `17 passed`，用于约束公会页面 helper 删除 `ValueError` fallback 后，业务异常仍按 `GameError` 正常提示，而 legacy `ValueError` 与科技升级成员校验翻译边界不回退。
- `guilds/tasks.py` 的帮会科技日产出单项入口也已继续按阶段 3 收口内部契约：`process_single_guild_production()` 当前对“`failed_ids` 为空但 `guild_id` 缺失”的坏调用不再抛裸 `ValueError`，而是统一改走显式 `AssertionError`，避免后台任务入口继续把明显的调度契约错误伪装成普通参数校验失败。
- `gameplay/management/commands/bootstrap_game_data.py`、`gameplay/management/commands/load_item_templates.py` 与 `guilds/management/commands/check_guild_schema.py` 这批管理命令/辅助函数也已继续按阶段 3 收口 broad catch：`bootstrap_game_data` 当前只会把显式 `CommandError` 记为可汇总的步骤失败，`AssertionError("broken bootstrap step")` 这类编程错误会继续直接冒泡；`load_item_templates` 的图片导入当前只对显式 `OSError`/图片文件基础设施故障做 best-effort 警告，压缩/存储链里的契约错误不再被统一吞掉；`check_guild_schema` 的表结构 introspection 也已改成只对白名单内数据库基础设施异常降级返回 `None`，避免管理命令继续用 `except Exception` 掩盖 introspection 调用错误。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_bootstrap_game_data_command.py tests/test_load_item_templates_command.py tests/test_check_guild_schema_command.py -q` 为 `10 passed`，用于约束管理命令步骤汇总、物品图片导入与公会 schema introspection 的基础设施降级/编程错误冒泡边界不回退。
- `trade/services/cache_resilience.py` 的共享 cache helper 也已继续按阶段 3 退出 broad catch：`best_effort_cache_get/set/add/delete()` 当前直接改成显式 `infrastructure_exceptions` 白名单，只对调用方声明的缓存基础设施故障做 warning 级 best-effort 降级；`RuntimeError("cache down")`、`AssertionError("broken cache contract")` 一类模糊 runtime / 编程错误会继续直接冒泡，不再依赖“先 `except Exception` 再 runtime marker 判断”的旧写法。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_trade_cache_resilience.py -q` 为 `12 passed`，用于约束 trade 共享 cache helper 的基础设施降级与编程错误冒泡边界不回退。
- `accounts/utils.py` 的活动 session 缓存与登录锁适配层也已继续按阶段 3 收口实现细节：`_safe_get_cached_session_key()`、`_safe_set_cached_session_key()`、`_acquire_login_lock()` 与 `_release_login_lock()` 当前都已改成直接捕获显式 `CACHE_INFRASTRUCTURE_EXCEPTIONS` 白名单，不再保留“先 `except Exception` 再 `isinstance(...)` 判断”的 legacy 分支；`AssertionError("broken active-session cache read contract")`、`AssertionError("broken login lock release contract")` 一类缓存/锁契约错误会继续直接冒泡，避免单会话登录链继续掩盖内部适配层问题。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_accounts_utils.py -q` 为 `13 passed`，用于约束活动 session 缓存与登录锁适配层从 broad catch 收口到显式 cache 白名单后的基础设施降级/契约错误冒泡边界不回退。
- `accounts/utils.py` 的 `purge_other_sessions()` 与 `accounts/signals.py` 的登录同步 signal 也已继续按阶段 3 收口外层失败边界：当前只会把显式 `DATABASE_CACHE_INFRASTRUCTURE_EXCEPTIONS` 记为“单会话同步降级/失败”，`AssertionError("broken active session sync contract")`、`AssertionError("broken session save contract")` 这类编程/契约错误会继续直接冒泡，不再被外层 broad catch 伪装成普通降级。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_accounts_utils.py -q` 为 `18 passed`，用于约束单会话同步外层失败边界退出 broad catch 后，基础设施故障仍会记录 degradation，而编程错误不再被静默吞掉。
- `accounts/utils.py` 的 `_purge_sessions_fallback()` 也已继续按阶段 3 收口：遍历旧 session 的兼容清理当前只会对白名单 `DATABASE_CACHE_INFRASTRUCTURE_EXCEPTIONS` 做 warning 级 best-effort 跳过，`AssertionError("broken fallback session delete contract")` 这类 fallback 清理内部契约错误会继续直接冒泡，不再被旧的 broad catch 伪装成普通“脏 session 可忽略”场景。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_accounts_utils.py -q` 为 `20 passed`，用于约束单会话 fallback 清理从 broad catch 收口到显式基础设施白名单后的降级/契约错误边界不回退。
- `gameplay/services/raid/combat/battle.py` 的战斗后置消息/保护清理实现层也已继续按阶段 3 退出最后的 broad catch：`_send_raid_battle_messages()` 与 `_dismiss_marching_raids_if_protected()` 当前只对显式消息/基础设施异常做 degrade warning，编程错误改由 `try/finally` 驱动在返程任务派发后直接冒泡，不再依赖 `except Exception` 先记录再延后重抛。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_raid_combat_battle.py -q` 为 `37 passed`，用于约束 raid battle 后置消息/清理补偿彻底退出 broad catch 后，基础设施故障仍带 degraded 标记，而编程错误继续在返程派发后直接冒泡。
- `pyproject.toml` 的 mypy 治理名单也已继续按阶段 3 收缩：`battle.admin / views / management.commands.__init__ / load_troop_templates`，以及 `core.views.__init__`、`gameplay.management.commands.__init__`、`gameplay.views.__init__`、`guests.management.commands.__init__`、`guests.views.__init__` 这批包级模块当前也已从宽泛的 `*.views.* / *.management.commands.* / *.admin.*` 忽略名单里单独 carve out；连同 `accounts.views` 与 `guests.views.common / read_helpers / recruit_action_runtime / recruit_handlers / salary`，这一批页面运行时、模板加载命令与包级导出模块都已开始接受真实类型检查。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`python -m mypy battle/admin.py battle/views.py battle/management/commands/__init__.py battle/management/commands/load_troop_templates.py core/views/__init__.py gameplay/management/commands/__init__.py gameplay/views/__init__.py guests/management/commands/__init__.py guests/views/__init__.py accounts/views.py guests/views/common.py guests/views/read_helpers.py guests/views/recruit_action_runtime.py guests/views/recruit_handlers.py guests/views/salary.py` 已通过，用于约束本轮继续缩减 mypy ignore 名单后，这批 battle / accounts / guests 模块与包级导出文件仍能维持真实类型检查通过。
- `pyproject.toml` 的 mypy 治理名单也已继续按阶段 3 收缩：`battle_debugger.__init__ / config / simulator / urls / views / management.__init__ / management.commands.__init__ / management.commands.battle_debug`、`gameplay.admin.__init__`、`guests.management.commands.test_recruitment`、`guests.templatetags.__init__ / guest_extras`，以及 `guilds.management.commands.__init__ / guilds.views.__init__` 当前也已从宽泛忽略名单里单独 carve out；这样阶段三不再只覆盖主业务模块，连调试工具、模板 tag 与包级导出边角模块也开始回到真实类型检查。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`python -m mypy battle_debugger/__init__.py battle_debugger/config.py battle_debugger/simulator.py battle_debugger/urls.py battle_debugger/views.py battle_debugger/management/commands/__init__.py battle_debugger/management/commands/battle_debug.py gameplay/admin/__init__.py guests/management/commands/test_recruitment.py guests/templatetags/__init__.py guests/templatetags/guest_extras.py guilds/management/commands/__init__.py guilds/views/__init__.py` 已通过，用于约束本轮继续缩减 mypy ignore 名单后，这批 debug / templatetag / 包级边角模块仍能维持真实类型检查通过。
- `pyproject.toml` 的 mypy 治理名单也已继续按阶段 3 收缩：`gameplay.models.__init__ / arena / items / manor / missions / progression / pvp` 当前也已从 `gameplay.models.*` 的宽泛 `ignore_errors = true` 规则里单独 carve out，开始回到真实类型检查；这意味着阶段三的宽泛 ignore 清单当前已经不再覆盖项目里的实际生产模块，只剩模式规则本身还保留在配置中。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`python -m mypy gameplay/models/__init__.py gameplay/models/arena.py gameplay/models/items.py gameplay/models/manor.py gameplay/models/missions.py gameplay/models/progression.py gameplay/models/pvp.py` 已通过，用于约束本轮继续缩减 mypy ignore 名单后，核心 gameplay 模型模块仍能维持真实类型检查通过。
- `pyproject.toml` 里的那条历史宽泛 `ignore_errors = true` override 当前也已直接删除：因为 `*.views.* / *.management.commands.* / *.admin.* / battle_debugger.* / gameplay.models.* / guests.templatetags.*` 这些模式如今都已有更细粒度的显式 carve-out 或已不再命中任何生产模块，继续保留只会制造“配置上还在豁免”的假象；阶段 3 到这里已经从“逐步 carve-out”进入“删除空转豁免规则”的收尾阶段。
- `pyproject.toml` 的类型门禁也已继续从“允许已标注模块通过”往“禁止新旧未标注函数”推进：`guilds.views.helpers`、`guilds.views.hero_pool` 与 `guilds.views.technology` 当前已加入 `disallow_untyped_defs = true` 的严格检查名单；配套视图辅助函数和页面入口签名已经补齐显式注解，避免这批阶段 3 已收口的 guild 视图重新退回“类型能过但函数签名继续裸奔”的状态。
- `pyproject.toml` 的严格门禁已继续扩到 `guilds.views.announcement / contribution / core`：这批页面入口函数当前也已补齐显式 `request / guild_id / HttpResponse` 签名，并加入 `disallow_untyped_defs = true` 严格名单；这样 `guilds` 视图域开始整体进入“禁止未标注函数”的阶段 3 精修状态，而不再只覆盖个别 helper。
- 截至 `2026-03-23`，阶段 3 的两条主目标已经具备收尾条件：生产代码里的宽泛 `except Exception` 扫描结果为 `0`，历史宽泛 `ignore_errors = true` 规则也已删除且命中剩余模块数为 `0`；后续若继续推进，主要属于“逐模块提升 `disallow_untyped_defs` 严格度”的精修项，而不再是阶段 3 主线阻塞。
- 截至 `2026-03-23`，阶段 3 的默认门禁复核也已再次通过：`make lint` 通过，默认 `make test` 通过，结果为 `2350 passed, 38 deselected`；这意味着阶段 3 的异常/类型治理不只停留在局部回归，而是已重新回到默认快速门禁绿灯。
- `2026-03-23` 已启动阶段 4 首轮治理：`guests/templates/guests/detail.html` 这份当前最大模板已先把页面专属交互脚本迁移到 `static/js/guest-detail.js`，详情页模板不再内联“属性加点 AJAX / 技能书弹窗 / 装备弹窗与装备 AJAX”这组页面状态机；对应回归 `tests/test_guest_runtime_refresh_views.py`、`tests/test_guest_allocate_points_view.py`、`tests/test_guest_view_error_boundaries.py` 与 `tests/test_guest_item_view_validation.py -k "gear_options"` 已通过，用于约束脚本外迁后读路径、属性加点和装备读取边界不回退。
- `2026-03-23` 已继续推进阶段 4 第二刀：`guests/templates/guests/roster.html` 的工资弹窗、一键支付确认、经验道具与疗伤 AJAX 逻辑已迁移到 `static/js/guest-roster.js`，门客列表页模板不再内联整段页面状态脚本；并补 `tests/test_guest_runtime_refresh_views.py` 约束 `roster` 页面继续走外链脚本边界，避免后续把页面逻辑重新塞回模板。
- `2026-03-23` 阶段 4 第二刀聚焦回归已通过：`pytest tests/test_guest_runtime_refresh_views.py tests/test_salary_views.py tests/test_guest_item_view_validation.py -k "use_exp_item or use_medicine_item or roster or detail or salary" -q` 结果为 `23 passed, 29 deselected`，用于约束门客列表页脚本外迁后工资视图、经验道具与疗伤交互边界不回退。
- `2026-03-23` 已继续推进阶段 4 第三刀：`gameplay/templates/gameplay/warehouse.html` 的 AJAX 表单、门客选择弹窗和灵魂容器筛选逻辑已迁移到 `static/js/warehouse-page.js`，并同步清理 `warehouse` 模板中的 `onclick` / `onchange` 历史内联事件，页面脚本改为通过 `data-*` 属性接管操作 URL 与动作类型。
- `2026-03-23` 阶段 4 第三刀聚焦回归已通过：`pytest tests/test_inventory_views.py -k "warehouse or move_item_to_warehouse or move_item_to_treasury or use_rebirth_card or use_xisuidan or use_xidianka or use_guest_rarity_upgrade or use_soul_container or use_item_ajax" -q` 结果为 `26 passed, 5 deselected`，用于约束 `warehouse` 页面脚本外迁后仓库/藏宝阁搬运、目标门客道具和脚本边界不回退。
- `2026-03-23` 已继续推进阶段 4 第四刀：`gameplay/templates/gameplay/recruitment_hall.html` 的招募确认、候选区分块渲染、候选区片段替换和放大镜 AJAX 逻辑已迁移到 `static/js/recruitment-hall.js`；同时把候选区选中集合统一收口到隐藏字段同步，避免分块渲染时批量提交遗漏未渲染候选。
- `2026-03-23` 阶段 4 第四刀聚焦回归已通过：`pytest tests/test_inventory_views.py tests/test_guest_item_view_validation.py -k "recruitment_hall or recruit_view_ajax or candidate_accept_view or use_magnifying_glass_view" -q` 结果为 `17 passed, 49 deselected`，用于约束 `recruitment_hall` 页面脚本外迁后招募、候选处理和放大镜交互边界不回退。
- `2026-03-23` 已继续推进阶段 4 第五刀：`gameplay/templates/gameplay/messages.html` 的清空确认、附件领取和消息已读 AJAX 逻辑已迁移到 `static/js/messages-page.js`，并顺手修正消息页前端已读状态更新仍引用旧 `.msg-item / .msg-unread-dot` 结构的漏口，使其与当前表格 DOM 保持一致。
- `2026-03-23` 阶段 4 第五刀聚焦回归已通过：`pytest tests/test_message_views.py -q` 结果为 `14 passed`，用于约束消息页脚本外迁后列表渲染、附件领取和已读 JSON 交互边界不回退。
- `2026-03-23` 已继续推进阶段 4 第六刀：`gameplay/templates/gameplay/tasks.html` 的任务标签切换、门客人数限制和护院滑块/数字输入联动逻辑已迁移到 `static/js/tasks-page.js`，任务页模板不再内联这段页面交互脚本。
- `2026-03-23` 阶段 4 第六刀聚焦回归已通过：`pytest tests/test_mission_views.py -q` 结果为 `28 passed`，用于约束任务页脚本外迁后页面渲染与接受任务视图边界不回退。
- `2026-03-23` 已继续推进阶段 4 第七刀：`templates/landing.html`、`gameplay/templates/gameplay/map.html` 与 `gameplay/templates/gameplay/message_detail.html` 的页面交互脚本已分别迁移到 `static/js/landing-page.js`、`static/js/map-page.js` 与 `static/js/message-detail-page.js`；其中首页的移动端折叠/撤退确认、地图页的搜索/侦察 AJAX，以及消息详情页的删除确认都已退出模板内联逻辑，地图页还顺手清理了已无对应 DOM 的历史撤退死代码。
- `2026-03-23` 阶段 4 第七刀聚焦回归已通过：`pytest tests/test_core_views.py -k "home_page_raid_scout_countdowns_use_explicit_refresh_api or home_page_uses_external_landing_script_for_retreat_and_collapse_actions" -q` 为 `2 passed, 19 deselected`；`pytest tests/test_map_views.py tests/test_message_views.py -q` 为 `57 passed`，用于约束首页、地图页与消息详情页脚本外迁后页面渲染、侦察/撤退入口与消息详情交互边界不回退。
- `2026-03-23` 已继续推进阶段 4 第八刀：`gameplay/templates/gameplay/smithy.html`、`gameplay/templates/gameplay/stable.html` 与 `gameplay/templates/gameplay/ranch.html` 这组三连页已把数量成本预览逻辑统一迁移到共享外链 `static/js/production-cost-preview.js`，模板中的 `onchange="updateTotalCost(...)"` 和同名全局函数已全部移除。
- `2026-03-23` 阶段 4 第八刀聚焦回归已通过：`pytest tests/test_production_views.py -q` 结果为 `23 passed`，用于约束冶炼坊/马房/畜牧场页面脚本共享化后渲染与生产入口边界不回退。
- `2026-03-23` 已继续推进阶段 4 第九刀并完成封板：`gameplay/templates/gameplay/work.html`、`gameplay/templates/gameplay/forge.html`、`gameplay/templates/gameplay/raid_config.html` 与 `gameplay/templates/gameplay/arena/registration.html` 的页面交互脚本已迁移到 `static/js/work-page.js`、`static/js/forge-page.js`、`static/js/raid-config-page.js` 与 `static/js/arena-registration.js`；配合前六刀，阶段 4 范围内模板内联脚本与内联事件扫描结果已清零。
- `2026-03-23` 阶段 4 封板验证已通过：`pytest tests/test_work_views.py tests/test_forge_views.py tests/test_map_views.py tests/test_arena_views.py -q` 结果为 `108 passed`；`rg -n "<script>(?!.*application/ld\\+json)|onclick=|onchange=|onsubmit=|oninput=|onblur=|onfocus=" templates gameplay/templates guests/templates -g '*.html' -P` 无输出，说明阶段 4 当前工作区目标已满足“前端交互逻辑不再散落在模板内联代码中”的完成标志。
- `2026-03-23` 已完成阶段 4 最后一轮样式收口：`templates/landing.html` 与 `gameplay/templates/gameplay/message_detail.html` 的页面级样式已进一步迁移到 `static/css/landing-page.css` 与 `static/css/message-detail.css`，对应模板中的大段 `<style>` 与相关 `style=` 历史残留已一并清理。
- `2026-03-23` 阶段 4 最终收口验证已通过：`pytest tests/test_core_views.py -k "home_page_uses_external_landing_script_for_retreat_and_collapse_actions" -q` 与 `pytest tests/test_message_views.py -k "message_detail_page_loads_external_page_script_without_inline_logic" -q` 均通过；`rg -n "<style>|style=\"" templates/landing.html gameplay/templates/gameplay/message_detail.html` 无输出，说明阶段 4 不只完成脚本外迁，也已把首页/消息详情页这两处最后的明显页面级样式热点同步收口。
- `2026-03-23` 已继续补一轮阶段 3 收尾验证：`python -m mypy` 覆盖当前工作区修改的 `accounts / battle_debugger / core / gameplay / guests / guilds / trade` 相关文件已通过；`pytest tests/test_accounts_utils.py tests/test_accounts.py tests/test_guild_view_helpers.py tests/test_guild_hero_pool_views.py tests/test_guilds_tasks.py tests/test_guilds_technology_service.py tests/test_raid_combat_battle.py tests/test_bootstrap_game_data_command.py tests/test_load_item_templates_command.py tests/test_check_guild_schema_command.py tests/test_reload_runtime_configs_command.py tests/test_validate_yaml_configs_command.py tests/test_cleanup_data_report_command.py tests/test_management_command_validation.py tests/test_trade_cache_resilience.py tests/test_trade_bank_service.py tests/trade_auction_rounds/round_lifecycle.py tests/test_guest_template_filters.py tests/test_global_mail_admin.py -q` 为 `204 passed`，用于约束阶段 3 收尾前的关键异常边界、类型门禁与管理命令/admin/view 回归不回退。
...
- `resources` 服务也已继续按阶段 3 收口：`gameplay/services/resources._handle_unknown_resource()` 在 debug 下对未知资源类型改走显式内部调用契约错误 `AssertionError`，非 debug 环境继续记录错误并跳过非法资源；与此同时，`grant_resources* / spend_resources* / log_resource_gain()` 当前也开始对 `cost / rewards / event payload` 本身的形状、空 key、布尔值数量与负数数量做显式契约校验，不再静默退化成空映射、`0` 或合法整数，避免资源发放/扣除链在坏输入下悄悄少扣、少发或无声跳过。
...
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_gameplay_services.py -k "grant_resources or spend_resources or sync_resource_production" -q` 为 `16 passed, 13 deselected`，用于约束 `resources` 服务在坏掉的 `cost / rewards` payload、布尔值数量和负数数量输入下改走显式 `AssertionError`，不再静默把坏输入当空映射、`0` 或合法数值处理。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_guest_item_view_validation.py tests/test_guest_view_error_boundaries.py -k "use_medicine_item_view or learn_skill_view or forget_skill_view" -q` 为 `19 passed, 55 deselected`，用于约束门客药品/技能页面入口在坏掉的 `effect_payload / skill_key` 配置下继续稳定走 400/错误提示，且不会回退成静默兼容或运行时形状错误。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_guest_runtime_refresh_views.py -q` 为 `5 passed`，用于约束门客详情 GET 读路径在坏掉的技能书 `effect_payload / skill_key` 配置下继续直接暴露契约错误，而不会静默少展示技能书或在 GET 中隐式纠偏。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_guest_item_view_validation.py tests/test_guest_view_error_boundaries.py -k "use_experience_item_view or allocate_points_view or train" -q` 为 `15 passed, 60 deselected`，用于约束门客训练/经验道具/属性加点页面入口在坏掉的经验道具 `effect_payload` 配置下继续稳定走 400/错误提示，且不会回退成静默兼容或运行时形状错误。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_inventory_views.py -k "use_rebirth_card or use_xisuidan or use_xidianka or use_guest_rarity_upgrade or use_soul_container" -q` 为 `10 passed, 20 deselected`，用于约束仓库目标门客道具入口在坏掉的 `effect_payload` 配置下继续直接暴露契约错误，而不会静默把坏 payload 当空配置或错判成普通物品类型错误。
...
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_mission_sync_report.py -q` 为 `16 passed`，用于约束 `mission` 同步战报 helper 在 defense/offense 场景下不再静默吞掉坏掉的 `enemy_guests / enemy_troops / guest_level / drop_table / enemy_technology / guest_skills` 配置，而是改走显式 `AssertionError`。
...
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_battle_salvage.py -q` 为 `10 passed`，用于约束 `battle salvage` 计算链在坏掉的 `report.losses / casualty entry / troop template / team member / equipment_recovery` 载荷下改走显式 `AssertionError`，不再静默少发经验果或装备回收。
...
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_mission_refresh_async.py -q` 为 `32 passed`，用于约束 `mission` launch helper、refresh 配置 helper 与完成任务调度入口在 offense 场景下不再静默吞掉坏掉的 `enemy_guests / enemy_troops / enemy_technology / drop_table / MISSION_REFRESH_SYNC_MAX_RUNS / return_at` 配置，而是改走显式 `AssertionError` 或显式契约错误。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_battle_ai_generator_contracts.py tests/test_battle_tasks_generate_report_task.py -q` 为 `20 passed`，用于约束 `mission` 依赖的 battle 异步战报任务与 AI 门客生成层在坏掉的 `manor_id / mission_id / run_id / guest_ids / battle_type / travel_seconds / level` 输入下统一改走显式 `AssertionError`，不再把坏任务参数留给 ORM、默认战斗类型或下游 battle 层做隐式兜底。
- `2026-03-23` 已继续补一轮阶段 3 聚焦验证：`pytest tests/test_battle.py -k "snapshot" -q` 为 `19 passed, 24 deselected`，用于约束 `mission` 依赖的 battle snapshot 链在坏掉的 `display_name / rarity / status / current_hp` 以及坏快照 payload 下统一改走显式 `AssertionError`，不再静默补默认文本或夹正血量。
...
- `guests/views/items.py` 与 `guests/views/skills.py` 的 payload 读取边界也已继续按阶段 3 收口：药品页当前对非 dict `effect_payload` 不再依赖 `or {}` 静默退化成空配置，技能书页对非 dict `effect_payload` 和非字符串 `skill_key` 也不再依赖 `str(...)` 强转或静默回空；这些坏配置现在统一改走显式 `GuestItemConfigurationError` / 错误提示，避免页面链在坏模板配置下偶发 `AttributeError` 或悄悄忽略技能书内容。
- `guests/views/roster.py` 的详情页技能书读侧边界也已继续按阶段 3 收口：详情页当前在构建 `skill_books` 上下文时，不再把坏掉的技能书 `effect_payload` 或非字符串 `skill_key` 静默跳过并少展示候选技能书，而是统一改走显式 `AssertionError`，避免 GET 读路径把模板配置损坏伪装成“只是没有可学技能书”。
- `guests/views/training.py` 的经验道具 payload 读取边界也已继续按阶段 3 收口：经验道具入口当前对非 dict `effect_payload` 不再依赖 `or {}` 静默退化成空配置，而是统一改走显式 `GuestItemConfigurationError` / 错误提示，避免训练页在坏模板配置下偶发 `AttributeError` 或悄悄忽略经验道具的时间配置。
...
- `mission` 的 loadout 数量与 key 契约也已继续按阶段 3 收口：`gameplay/utils/resource_calculator.py` 当前对已知兵种的坏数量值不再静默归零，对 `calculate_travel_time()` 里的未知正数兵种 key 也不再默认为基础速度，而是统一改走显式 `AssertionError`，避免发起链和旅行时间计算在坏载荷下悄悄改写兵力与耗时语义。
...
- `mission` 的 guest config dict 结构契约也已继续按阶段 3 收口：`execution_adapters.normalize_guest_configs()` 与 `sync_report.py` 当前对 `enemy_guests` 中缺失/空白 `key` 的 dict 条目不再无条件放行，而是统一改走显式 `AssertionError`，避免坏配置在发起和同步战报链里混入无效敌方门客条目。
- `mission` 的 guest config skills 契约也已继续按阶段 3 收口：`execution_adapters.normalize_guest_configs()` 与 `sync_report.py` 当前对 `enemy_guests` dict 条目中的可选 `skills` 不再无条件放行，而是要求为非空字符串列表；坏类型或坏条目统一改走显式 `AssertionError`，避免发起和同步战报链在坏配置下混入无效技能覆盖。
- `mission` 的异步战报生成路径也已继续按阶段 3 与同步路径对齐：`battle/tasks.generate_report_task` 当前不再静默吞掉坏掉的 `enemy_technology / enemy_guests / enemy_troops / guest_skills` 配置，而是统一改走显式 `AssertionError`，避免 async 报告链与 sync_report 出现一边严格、一边静默兼容的语义分叉。
- `mission` 依赖的 AI 门客生成层也已继续按阶段 3 收口：`battle/combatants_pkg/ai_generator.py` 当前对坏掉的 `enemy_guests` 条目、坏掉的 `skills` 覆盖以及缺失模板 key 不再静默跳过，而是统一改走显式 `AssertionError`，避免 battle 层重新把上游已收紧的配置契约放宽回“best-effort 跳过”。
- `mission` 依赖的 AI 门客等级契约也已继续按阶段 3 收口：`battle/combatants_pkg/ai_generator.py` 当前对坏掉的 `level`（含布尔值、非正数）不再留给 `Guest(level=...)` 或后续成长计算隐式兜底，而是统一改走显式 `AssertionError`，避免 battle 层在坏敌方等级输入下悄悄生成畸形 AI 门客。
- `mission` 的异步战报快照载荷契约也已继续按阶段 3 收口：`battle/tasks.generate_report_task` 当前对坏掉的 `guest_snapshots` 容器或条目不再静默回空或留给下游爆出模糊异常，而是统一改走显式 `AssertionError`，避免 async 报告链在坏快照载荷下暴露不稳定异常语义。
- `mission` 的异步战报 offense 输入契约也已继续按阶段 3 与同步路径对齐：`battle/tasks.generate_report_task` 当前对 offense 分支传入的坏 `troop_loadout / defender_setup` 不再原样下传给 battle 层，而是统一改走显式 `AssertionError`，避免 async 报告链继续保留“同步严格、异步宽松”的输入边界分叉。
- `mission` 的异步战报任务入口契约也已继续按阶段 3 收口：`battle/tasks.generate_report_task` 当前对坏掉的 `manor_id / mission_id / run_id / guest_ids / battle_type / travel_seconds` 不再留给 ORM 查询、默认 battle config 或下游时间线构建隐式纠偏，而是统一改走显式 `AssertionError`，避免任务入口继续暴露不稳定的“有时返回 None、有时默认乱军试炼”的语义。
- `mission` 依赖的 battle snapshot proxy 层也已继续按阶段 3 收口：`gameplay/services/battle_snapshots.py` 当前对空 snapshot 或非 dict snapshot 不再静默跳过，而是统一改走显式 `AssertionError`，避免 snapshot 构建链在坏载荷下悄悄少人。
- `mission` 依赖的 battle snapshot 字段契约也已继续按阶段 3 收口：`gameplay/services/battle_snapshots.py` 当前对坏掉的 `template_key / skill_keys` 不再静默兜底成默认值或空列表，而是统一改走显式 `AssertionError`，避免 snapshot proxy 在坏载荷下制造伪造模板或悄悄丢失技能覆盖。
- `mission` 依赖的 battle snapshot 身份字段契约也已继续按阶段 3 收口：`gameplay/services/battle_snapshots.py` 当前对 `include_guest_identity=True` 场景下坏掉的 `guest_id / manor_id` 不再用 `or 0` 静默兜底，而是统一改走显式 `AssertionError`，避免 snapshot proxy 在坏载荷下伪造匿名身份。
- `mission` 依赖的 battle snapshot 技能序列化契约也已继续按阶段 3 收口：`gameplay/services/battle_snapshots.py` 当前对坏掉的 `skills.values_list(...) / _override_skills / skill_keys` 不再静默 `str(...)` 强转或丢弃，而是统一改走显式 `AssertionError`，避免 snapshot 构建链在坏技能载荷下悄悄丢失或伪造技能 key。
- `mission` 依赖的 battle snapshot 数值字段契约也已继续按阶段 3 收口：`gameplay/services/battle_snapshots.py` 当前对 `level / current_hp / attack / defense / max_hp / troop_capacity` 不再用 `or 0/1` 或 clamp 静默兜底，而是统一改走显式 `AssertionError`，避免 snapshot proxy 在坏载荷下悄悄改写战斗数值语义。
- `mission` 依赖的 live guest snapshot 构建契约也已继续按阶段 3 收口：`gameplay/services/battle_snapshots.py` 当前对坏掉的 `guest.id / guest.manor_id / guest.template.key` 不再直接 `int(...)` / 透传字符串并留给下游兜底，而是统一改走显式 `AssertionError`，避免 live guest 快照在坏对象下生成伪造身份或空模板 key。
- `mission` 依赖的 battle snapshot 文本与血量契约也已继续按阶段 3 收口：`gameplay/services/battle_snapshots.py` 当前对坏掉的 `display_name / rarity / status` 不再静默补成“无名门客 / gray / idle”，对 live guest 的 `current_hp` 也不再夹到 `1..max_hp`；这些情况现在统一改走显式 `AssertionError`，避免 snapshot 链在坏对象或坏快照下悄悄改写展示文本和参战血量语义。
...
- `mission` 的结算 HP 数值契约也已继续按阶段 3 收口：`gameplay/services/missions_impl/finalization_helpers.py` 当前对 `hp_updates / remaining_hp` 中的负数不再静默留给后续 clamp 逻辑纠偏，而是统一改走显式 `AssertionError`，避免结算链路在坏战报 payload 下悄悄改写门客血量语义。
- `mission` 的结算 guest_id 契约也已继续按阶段 3 收口：`gameplay/services/missions_impl/finalization_helpers.py` 当前对 `hp_updates / team_entries` 中的非正 `guest_id` 不再继续带入结算链，而是统一改走显式 `AssertionError`，避免坏战报 payload 在结算时制造无效参战门客 ID。
...
- `mission / raid` 共用的 battle salvage 契约也已继续按阶段 3 收口：`gameplay/services/battle_salvage.py` 当前对坏掉的 `report.losses`、`casualty key/lost`、门客 team entry、troop template 的 `recruit/base_duration/equipment` 以及发奖阶段的 `equipment_recovery` 不再静默兜底成 `0 / [] / gray / 默认时长` 或悄悄跳过，而是统一改走显式 `AssertionError`，避免 salvage 链在坏战报或坏配置下少发经验果、少发装备却不暴露问题。
...
- `mission` 的额外次数计数布尔值契约也已继续按阶段 3 收口：`gameplay/services/missions_impl/attempts.py` 当前对 `add_mission_extra_attempt(True/False)` 这类布尔值不再沿用 Python `bool -> int` 的隐式转换，而是统一改走显式 `AssertionError`，避免次数服务在坏调用下悄悄加 1 或加 0。
...
- `guest equipment` 的模板预览/同步契约也已继续按阶段 3 收口：`guests/services/equipment.py` 当前对坏掉的 `item_template.effect_payload / item_template.key / item_template.effect_type`、未知 `extra_stats` 键、坏掉的 `set_bonus`、非法 `choice` 与负库存数量不再静默 `str(...)` 强转、过滤或夹成 `0`，而是统一改走显式 `AssertionError`，避免装备预览、库存同步与穿戴链在坏配置/坏输入下悄悄少加属性、少生成装备或错选装备。
- `guest equipment` 的已持久化属性契约也已继续按阶段 3 收口：`guests/services/equipment.py` 当前在应用/撤销装备属性和套装效果时，会显式校验 `extra_stats` 与 `gear_set_bonus` 的 key/value 形状；坏掉的已持久化装备模板或门客套装加成不再静默按 `0` 跳过，而是统一改走显式 `AssertionError`，避免装备/辞退/灵魂容器链在脏状态下悄悄改写属性。
...
- `inventory` 的目标门客道具 view 边界也已继续按阶段 3 收口：`gameplay/views/inventory.py` 当前对重生卡 / 升阶道具 / 灵魂容器 / 洗髓丹 / 洗点卡这批入口，不再把非 dict `effect_payload` 静默当成空配置再误判成普通“物品类型错误”；坏 payload 现在直接暴露为显式 `AssertionError`，避免页面链在模板配置损坏时把真实契约错误伪装成普通用户操作错误。
...
- `inventory` 的召唤卡条目/消耗明细契约也已继续按阶段 3 收口：`gameplay/services/inventory/use.py` 当前对 `choices.template_key`、`required_items` 的 item key 与数量不再静默 `str(...)` 强转或把 `0/负数` 当成“无需消耗”跳过，而是统一改走显式 `ItemNotConfiguredError`，避免召唤卡链路在坏配置下悄悄免费使用或引用伪造道具键。
...
- `inventory` 的宝箱奖励列表条目契约也已继续按阶段 3 收口：`gameplay/services/inventory/use.py` 里 `gear_keys / skill_book_keys` 当前不仅要求为列表，也要求每个条目都是非空字符串；坏条目不再静默带进查询或发奖链，而是统一改走显式 `ItemNotConfiguredError`，避免宝箱奖励链在坏配置下悄悄少发或引用伪造物品键。
...
- `inventory` 的唯一门客列表条目契约也已继续按阶段 3 收口：`gameplay/services/inventory/use.py` 里 `exclusive_template_keys` 当前不仅要求为列表，也要求每个模板键都是非空字符串；坏条目不再静默跳过并削弱唯一门客保护，而是统一改走显式 `ItemNotConfiguredError`。
