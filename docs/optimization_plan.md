...
- `mission / raid / guest recruitment` 的真实并发语义测试仍然不够；虽然 `mission` 发起/撤退、`raid` 发起、`guest recruitment` 发起已覆盖首批 `select_for_update` 竞争，但任务派发、worker 实际消费和 refresh 补偿链路仍是缺口。
